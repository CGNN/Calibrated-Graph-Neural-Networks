...
def generateUnseen(num_class, num_unseen):
    if type(num_class) == int:
        return combinations(range(num_class), num_unseen)
    else:
        return combinations(num_class, num_unseen)
...
def createOGBTraining(labels, idx_train, idx_val, idx_test, max_train=20, balance=True, new_classes=[]):
    ...
    for k in dist:
        ...
        cluster_mapping[k] = len(cluster_mapping)
    if False:
        for idx in idx_train:
        ...
        for k in dist:
            ...
            if max_train < len(dist[k]):
                new_idx_train += np.random.choice(dist[k], max_train).tolist()
            else:
    ...
    for idx,label in enumerate(labels):
        if label in new_mapping:
            ...
            if label in cluster_mapping:
                ...
                labels[idx] = -1
        if label in cluster_mapping:
            cluster_labels.append(cluster_mapping[label])
        else:
            cluster_labels.append(-1)
    #print(cluster_mapping, new_mapping)
    return new_idx_train, new_idx_val, in_idx_test, new_idx_test, out_idx_test, labels, cluster_labels

def createDBLPTraining(labels, idx_train, idx_val, idx_test, max_train=20, balance=True, new_classes=[]):
